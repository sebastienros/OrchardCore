...
| [Indexing](../../src/OrchardCore.Modules/OrchardCore.Indexing/Manifest.cs) / `OrchardCore.Indexing` | Bounded provider/source and profile discovery | `indexes list/show`, `indexes providers list` | Shared profile/provider coordination serves admin, recipes and Lucene definitions. Observable reset/synchronize/rebuild remains the next B07 slice; other provider adapters remain gated. | B07 |
