...
| Deployment | Shared plan/step, artifact and queued operation APIs/commands; separate private API-key import path | Owned artifacts and queued export/import are implemented and live-verified; seven feature-owned switch/selector adapters are implemented; remaining factories need a B08 audit. | [Deployment controllers](../../src/OrchardCore.Modules/OrchardCore.Deployment/Controllers), [remote import](../../src/OrchardCore.Modules/OrchardCore.Deployment.Remote/Controllers/ImportRemoteInstanceController.cs) |
...
| [Seo](../../src/OrchardCore.Modules/OrchardCore.Seo/Manifest.cs) / `OrchardCore.Seo` | Shared SEO content payload | Shared content commands | SeoMetaPart travels with content; typed robots settings expose ownership and public provider behavior. | B10 |
...
| [BackgroundTasks](../../src/OrchardCore.Modules/OrchardCore.BackgroundTasks/Manifest.cs) / `OrchardCore.BackgroundTasks` | Registered tasks, validation, configuration and enable/disable | `background-tasks` | Shared existing admin actions and scheduler settings signals; no invented run/cancel lifecycle. Locally verified, pending PR merge. | B11 |
