...
| [RateLimits](../../src/OrchardCore.Modules/OrchardCore.RateLimits/Manifest.cs) / `OrchardCore.RateLimits` | Policy/limiter lifecycle and four built-in source schemas | `rate-limits policies`, `rate-limits policies limiters`, `rate-limits limiter-types` | Shared admin/recipe validation and policy mutations; runtime 429/recovery verified locally. Pending PR validation/merge. Host-contributed limits remain in code. | B05 |
