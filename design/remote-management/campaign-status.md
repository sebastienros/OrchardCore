...
| Deployment plans, export and import | Plan/step PR merged; artifacts in progress | [PR #28](https://github.com/sebastienros/OrchardCore/pull/28) merged as `c037e5835` at exact green head `fbcc29684`; full Linux/Windows CI, local server/CLI/MCP, live transports and package 0.10.27 validation passed. [Plan/step evidence](deployment-plans.md). Independent artifact branch now integrates the merged target: shared existing export archive and local import staging services, full server suite (3,495 passed plus CI-only skip), 28 focused controller/staging tests and [artifact contract](deployment-artifacts.md). Durable artifacts/operations, Pomi binary transport, cross-tenant round-trip and further adapters remain. |
