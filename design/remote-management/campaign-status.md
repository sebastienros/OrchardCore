...
| Deployment plans, export and import | Plan/step slice in progress | Independent `codex/remote-deployment-plans` now integrates merged `2eaa672ce`. [Caller audit and contract](deployment-plans.md); two baseline regressions corrected. Shared admin/API plan CRUD, shared step mutations and four explicit built-in configuration contracts are implemented. Step-type discovery/schema endpoints and shared export type resolution are implemented. Persisted step CRUD/order endpoints are implemented. Generic settings and content adapters are implemented. Recipe replacement now shares batch preflight and built-in validation. Legacy step ID migration and recipe replay identity are implemented. Existing content-controller integration and disabled-feature restoration pass; the latter exposed and fixed missing document serializer options in unknown-type fallback. Strict build passes; full server suite: 3,475 passed plus one CI-only skip. Strict documentation build previously passed. Tenant isolation and live HTTP/Pomi/MCP smoke pass. Full solution strict build, CLI 295 and MCP 78 tests pass. Pomi packaging/distribution/help, inventory, final review and independent PR CI remain; additional built-in adapter coverage requires the B08 completion audit. |
