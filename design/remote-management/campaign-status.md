...
| Common index definitions and lifecycle | In development | Independent `codex/remote-index-definitions` from merged `b9361a1bb`; [domain audit and baseline](index-definitions.md). Incoming-update validation failed before and passes after a shared manager fix; strict server build and two focused regressions pass. Existing caller integration and index endpoints remain. Definitions/discovery and lifecycle remain separate B07 PRs. |
