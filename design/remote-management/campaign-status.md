...
| Deployment plans, export and import | Plan/step slice in progress | Independent `codex/remote-deployment-plans` now integrates merged `2eaa672ce`. [Caller audit and contract](deployment-plans.md); two baseline regressions corrected. Shared admin/API plan CRUD, shared step mutations and four explicit built-in configuration contracts are implemented. Step-type discovery/schema endpoints and shared export type resolution are implemented. Persisted step CRUD/order endpoints are implemented. Strict build and 31 focused tests pass, including real persistence and idempotent retries. Remaining adapters/recipe validation, identity/tenant/feature gates, live and final integration checks remain. |
