...
| Common index definitions and lifecycle | Definitions merged; lifecycle remains | [PR #24](https://github.com/sebastienros/OrchardCore/pull/24), merge `d26b9888b`; all CI green, including Linux/Windows. Common discovery, typed Lucene definitions, shared admin/recipe coordination, runtime indexing and tenant isolation verified. Observable lifecycle operations remain a separate subsequent PR. See [contract and evidence](index-definitions.md). |
| Observable index lifecycle | In progress | Shared worker/coordinator, persisted operations, admin/recipe routing and HTTP/Pomi/MCP commands implemented. Live Lucene synchronization/reset/rebuild and permission/provider gates pass. Six real-tenant recipe caller cases and six lifecycle coordinator cases pass; CLI 295/MCP 78 pass. Nine admin caller cases pass. Full server 3,399 passed (one CI-only skip), strict docs/distribution passed. Synchronization callbacks and legacy source compatibility now pass 17 focused coordinator/runner cases. Lease-expiry guards and uncertain outcomes pass 31 focused tests. Fresh HTTP/Pomi/MCP lifecycle checks, strict docs/distribution and 315 examples across 244 help pages pass. Local validation complete; independent PR/CI publication is next. See [contract and evidence](index-lifecycle.md). |
| Deployment plans, export and import | Plan/step slice in progress | Independent `codex/remote-deployment-plans` starts at merged `8bb1ff436`. [Caller audit and contract](deployment-plans.md); two existing service regressions fail before and pass after correction. Resource APIs, typed steps and execution remain separate delivery gates. |
