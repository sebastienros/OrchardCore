...
| Common index definitions and lifecycle | In development | Independent `codex/remote-index-definitions`, integrated with merged `7e2a48e89`; [contract and evidence](index-definitions.md). Typed definitions, shared coordination and strict handler failures implemented at `b99e0bc6d`; live HTTP/Pomi/MCP, indexed-content/named-query updates and Lucene feature lifecycle pass. Strict solution/docs, server 3,344 passed (one CI-only skip), CLI 295 and MCP 78. Cross-tenant isolation, remaining caller audit and packaging precede PR creation. |
