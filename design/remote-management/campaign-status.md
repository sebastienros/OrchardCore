...
| Layers legacy recipe caller reuse | In validation | Independent `codex/shared-layer-recipe-management` from merged `7e2a48e89`. Shared layer mutations and rule validation; three failing-before regressions now pass. Seven recipe cases cover partial updates, extension conditions and immutable roots; full server 3,288 passed (one CI-only skip), strict build/docs pass. Integrated verification and PR remain. See [caller audit](layer-recipe-reuse.md). |
...
| Observable index lifecycle | PR open | [PR #26](https://github.com/sebastienros/OrchardCore/pull/26), head `c9f6910d1`; independent target, local integration/live/docs/package checks passed. Frontend/docs/assets/CLI/Windows CI passed; Linux functional tests running. |
| Frontend search settings | In progress | Independent branch from merged `769dd97c9`; [contract and caller audit](search-settings.md) committed. Shared admin/API editing and page-title field implemented; strict build and nine initial regressions pass. Permission/feature/runtime and final package/integration checks remain. |
