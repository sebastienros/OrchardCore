...
| Layers legacy recipe caller reuse | In validation | Independent `codex/shared-layer-recipe-management` from merged `7e2a48e89`. Shared layer mutations and rule validation; three failing-before regressions now pass. Successful partial updates and extension conditions are covered; full verification and PR remain. See [caller audit](layer-recipe-reuse.md). |
...
| Common index definitions and lifecycle | In development | Independent `codex/remote-index-definitions`, integrated with merged `7e2a48e89`; [contract and evidence](index-definitions.md). Typed definitions, shared coordination, strict handler failures and existing admin/recipe caller reuse implemented at `ff9239fac`; live HTTP/Pomi/MCP, indexed-content/named-query updates, Lucene feature lifecycle and cross-tenant isolation pass. Strict solution, server 3,351 passed (one CI-only skip), CLI 295 and MCP 78. Pomi 0.10.24 instructions include index definitions; strict documentation and reproducible distribution checks pass; live help validates 311 examples across 239 pages after enabling their features. PR review/CI/merge remain. Observable lifecycle operations are a separate subsequent PR. |
