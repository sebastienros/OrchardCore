...
| Layer zone settings | In verification | Independent `codex/remote-layer-zone-settings` from merged target `e8e6fda18`; shared admin normalization and typed settings adapter. Strict build, regression suites, live HTTP/Pomi/MCP and skill distribution verified. |
