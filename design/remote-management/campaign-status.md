...
| Observable index lifecycle | PR open | [PR #26](https://github.com/sebastienros/OrchardCore/pull/26), head `c9f6910d1`; independent target, local integration/live/docs/package checks passed. Frontend/docs/assets CI passed; Linux/Windows/CLI checks running. |
| Frontend search settings | In progress | Independent branch from merged `769dd97c9`; [contract and caller audit](search-settings.md) committed. Shared admin/API settings validation and the missing page-title editor field are next. |
