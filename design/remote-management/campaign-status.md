...
| Common index definitions and lifecycle | In development | Independent `codex/remote-index-definitions`, integrated with merged `7e2a48e89`; [contract and evidence](index-definitions.md). Typed Lucene definitions and shared admin/recipe coordination implemented at `714ec59bf`; live HTTP/Pomi/MCP definition checks pass. Strict solution/docs, server 3,332 passed (one CI-only skip), CLI 295 and MCP 78. Indexed-content/named-query, lifecycle/isolation and remaining caller audit gates precede PR creation. |
