...
| Deployment plans, export and import | Plan/step PR merged; artifact PR in review | [PR #28](https://github.com/sebastienros/OrchardCore/pull/28) merged as `c037e5835` at exact green head `fbcc29684`; full Linux/Windows CI, local server/CLI/MCP, live transports and package 0.10.27 validation passed. [Plan/step evidence](deployment-plans.md). [Artifact PR #30](https://github.com/sebastienros/OrchardCore/pull/30) targets the base directly and integrates the merged Flow fix: shared admin export/import services, private owned artifacts, bounded uploads, Pomi file downloads, retention cleanup, durable worker execution and operation APIs. Live OAuth/Pomi/MCP transport and a cross-tenant settings/content/media round trip pass; full CLI suite has 297 passes and strict docs/builds pass. [Artifact evidence](deployment-artifacts.md). Final recovery/full-suite checks, skill/package/inventory updates, PR review/CI and remaining adapter audit are in progress. |
