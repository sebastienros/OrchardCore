...
The optional `OrchardCore.RemoteManagement.Mcp` feature exposes a stateless Streamable HTTP MCP server at each tenant's `/mcp` endpoint. It projects tools from the existing management API metadata and OpenAPI schemas, and reuses tenant OAuth bearer authentication and operation permissions. Its separate configuration action registers a PKCE-protected OpenID application using the client's callback URLs. See [MCP server configuration](../reference/modules/RemoteManagement/README.md#mcp-server) for client registration, discovery, and supported inputs.
