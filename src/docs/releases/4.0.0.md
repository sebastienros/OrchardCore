...
### Remote management client features

Pomi setup and its OpenAPI command metadata now belong to `OrchardCore.RemoteManagement.Cli`. Existing Pomi tenants must enable that feature. Shared `OrchardCore.RemoteManagement` setup no longer registers the CLI application or enables device flow. The `RemoteManagement` recipe configures shared authentication; use `RemoteManagementCli` for Pomi. Existing OpenID applications are preserved.
...
The optional `OrchardCore.RemoteManagement.Mcp` feature exposes a stateless Streamable HTTP MCP server at each tenant's `/mcp` endpoint. It projects tools from the existing management API metadata and OpenAPI schemas, and reuses tenant OAuth bearer authentication and operation permissions. Compatible clients discover OAuth and register their own PKCE-protected OpenID applications automatically, then ask users to sign in and consent. Manual callback registration remains available for other clients. See [MCP server configuration](../reference/modules/RemoteManagement/README.md#mcp-server) for client registration, discovery, and supported inputs.

### Orchard Core Remote Management CLI

The new `OrchardCore.RemoteManagement` feature and `pomi` CLI provide authenticated remote administration of Orchard Core tenants. The CLI uses OpenAPI metadata to discover commands contributed by each tenant's enabled features, exposes a `schema` verb for resources that accept request bodies, supports browser PKCE, device authorization, and client-credentials authentication, emits JSON by default, and locally caches the search index published by `docs.orchardcore.net`. A dedicated **Settings → Remote Management** page diagnoses and repairs the required OpenID Connect configuration.

OpenID application registrations now distinguish web and native application types. Native public clients support RFC 8252 loopback redirect URIs with ephemeral ports.
