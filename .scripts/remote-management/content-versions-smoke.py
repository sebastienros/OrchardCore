...
def oc(*args, body=None, status=None, failure=False):
...
oc('context', 'add', 'content-versions-smoke', state['url'], '--current')
...
first = oc('content', 'items', 'save', body={'ContentType': name, 'TitlePart': {'Title': 'Original version'}})
item_id, first_id = first['ContentItemId'], first['ContentItemVersionId']
second = oc('content', 'items', 'update', item_id, body={'ContentType': name, 'TitlePart': {'Title': 'Current publication'}})
second_id = second['ContentItemVersionId']
assert first_id != second_id
listing = oc('content', 'versions', 'list', item_id, '--take', '1')
assert listing['totalCount'] == 2 and listing['items'][0]['ContentItemVersionId'] == second_id
archived = oc('content', 'versions', 'show', first_id)
assert not archived['Latest'] and not archived['Published']
assert archived['TitlePart']['Title'] == 'Original version'
rendered = oc('content', 'versions', 'render', first_id, '--display-type', 'Detail')
assert rendered['contentItemVersionId'] == first_id and rendered['html'].strip()
restored = oc('content', 'versions', 'restore', first_id)
...
oc('content', 'versions', 'restore', first_id, status=409)
